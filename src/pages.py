...
select = pn.widgets.Select(name='Year', 
                           options = [1990,1995,2000,2005,2010,2015,2020])
interactive_flow= pn.bind(migration_flow, selected_year=select)
dict_autocomplete = pn.widgets.AutocompleteInput(name='Search country', 
                                                 options=countries_dict(), 
                                                 case_sensitive = False)
plots_country = pn.Column(select, interactive_flow)

plots_country.save("/Users/paulacadena/Git-Hub/CAPP30239-IP/www/plots_country.html", 
                   embed=True)
